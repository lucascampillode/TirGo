...
class SecuenciaBrazoTiago(object):
    ...
    # -------------------  SECUENCIA DE COGIDA -------------------
    # Trigger: /tirgo/dispense/ready = true
    # Publica: /tirgo/tiago/picked = true
    ...
    def _ejecutar_secuencia_coger(self):
        ...
        for paso in self.secuencia:
            ...
            # Antes de ir a POS_COGIDA abrimos la pinza (para poder cerrar sobre el envase)
            ...
            # Justo después de POS_COGIDA cerramos pinza (envase agarrado)
            if paso["nombre"] == "POSICION COGIDA":
    ...
    # -------------------  SECUENCIA DE DEJADA -------------------
    # Trigger: /tirgo/tiago/at_patient = true
    # Publica: /tirgo/tiago/delivered = true
    #
    # Se asume que el robot YA lleva el objeto en la mano (gripper cerrado).
    # En POS_COGIDA: abre pinza para soltar el envase.
    # Tras pasar POS_COGIDA, cuando llegue a la SEGUNDA_APROX de salida: cierra pinza.
    ...
    def _ejecutar_secuencia_dejar(self):
        ...
        for paso in self.secuencia:
            ...
            # Llegando a la POS_COGIDA con el objeto en la mano -> soltar
            if nombre == "POSICION COGIDA":
                self._ejecutar_paso(paso)
                rospy.sleep(0.5)
                # Abrir gripper para soltar el envase
                self._abrir_gripper()
                pasada_pos_cogida = True
                continue

            # Después de haber pasado por POS_COGIDA, cuando volvemos a SEGUNDA_APROX: cerrar pinza
