...
class DispenseProcessStateMachine:
    ...
    Máquina de estados del 'Proceso de dispensación' con:
      ...
      - SIMULACIÓN de tiempos para el resto de hardware.
    """

    PHASE_TEXT = {
        "MOVING_TO_DISPENSER": ("Desplazamiento al dispensador", "El robot se dirige al módulo de dispensación."),
        "DISPENSING": ("Dispensación del medicamento", "El sistema prepara y libera el envase con la medicación."),
        "PICKING_CONTAINER": ("Recogida del envase", "El robot recoge de forma segura el envase dispensado."),
        "RETURNING_TO_PATIENT": ("Regreso al paciente", "El robot se desplaza de nuevo hacia la ubicación del paciente."),
        "DELIVERING_TO_PATIENT": ("Entrega al paciente", "El robot entrega el medicamento al paciente."),
        "CLOSING_INTERACTION": ("Cierre de la interacción", "Se confirma la entrega y el robot se despide."),
    }
    ...
    def __init__(self) -> None:
        ...
        # --------- CONFIGURACIÓN ---------
        # Si True, el script simulará el hardware tras unos segundos.
        # (El movimiento de navegación se hace SIEMPRE con Follower)
        self.SIMULATION_MODE = True 
        self.SIM_DELAY = 3.0  # Segundos que tarda cada etapa en la simulación
        ...
        self.pub_dispense_ready = rospy.Publisher("/tirgo/dispense/ready", Bool, queue_size=1) # Necesario para simular
        self.pub_picked = rospy.Publisher("/tirgo/tiago/picked", Bool, queue_size=1)
        self.pub_at_patient = rospy.Publisher("/tirgo/tiago/at_patient", Bool, queue_size=1)
        self.pub_delivered = rospy.Publisher("/tirgo/tiago/delivered", Bool, queue_size=1)
    ...
    # ------------------------------------------------------------------
    #  Lógica de Simulación (para hardware no móvil)
    # ------------------------------------------------------------------

    def trigger_simulation_event(self, publisher):
        """
        Crea un Timer que publicará True en el topic dado tras self.SIM_DELAY segundos.
        Esto simula que el hardware ha terminado su tarea.
        """
        if self.SIMULATION_MODE:
            rospy.loginfo(f"...Simulando trabajo ({self.SIM_DELAY}s)...")
            rospy.Timer(
                rospy.Duration(self.SIM_DELAY), 
                lambda event: publisher.publish(Bool(data=True)), 
                oneshot=True
            )
    ...
    def set_phase(self, new_phase: str) -> None:
        ...
        rospy.loginfo(f"Fase actual: {title}")
        rospy.loginfo(desc)
        rospy.loginfo("====================================")
    ...
    def _launch_navigation(self, checkpoint, done_publisher, label: str):
        ...
        """
        def worker():
            rospy.loginfo(f"[NAV] Iniciando movimiento: {label}")
            # Follower espera a que llegue y se pare
            ok = self.follower.enviar_puntos([checkpoint])
            if ok:
                rospy.loginfo(f"[NAV] Movimiento '{label}' completado. Publicando True...")
                done_publisher.publish(Bool(data=True))
            else:
                rospy.logwarn(f"[NAV] Movimiento '{label}' NO se completó correctamente.")
    ...
    def cb_mission_start(self, msg: String) -> None:
        ...
        # Antes: simulación de llegada
        # self.trigger_simulation_event(self.pub_arrived)

        # Ahora: movimiento REAL al dispensador
    ...
    def cb_arrived(self, msg: Bool) -> None:
        ...
        # Simular: El dispensador tarda X segundos en preparar medicina (/dispense/ready)
        self.trigger_simulation_event(self.pub_dispense_ready)
    ...
    def cb_dispense_ready(self, msg: Bool) -> None:
        ...
        # Simular: El brazo tarda X segundos en coger el bote (/picked)
        self.trigger_simulation_event(self.pub_picked)
    ...
    def cb_picked(self, msg: Bool) -> None:
        ...
        # Antes: simulación de la vuelta
        # self.trigger_simulation_event(self.pub_at_patient)

        # Ahora: movimiento REAL de vuelta al paciente
        self._launch_navigation(
            self.checkpoint_paciente,
            self.pub_at_patient,
            label="al_paciente"
    ...
    def cb_at_patient(self, msg: Bool) -> None:
        ...
        # Simular: El robot habla e interactúa, tarda X segundos (/delivered)
        self.trigger_simulation_event(self.pub_delivered)
    ...
    def cb_delivered(self, msg: Bool) -> None:
        ...
        # Si quieres que vuelva a admitir otra misión automáticamente:
        # self.state = "IDLE"
        self.state = "IDLE"
...
def main():
    ...
    rospy.loginfo("Simulador listo. Ejecuta: rostopic pub /tirgo/mission/start std_msgs/String 'go' -1")
    rospy.spin()
